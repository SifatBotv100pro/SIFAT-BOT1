...
// ================= ADMINBOT ban protection =================
// Central guard: call this BEFORE writing any ban (userBanned Map / data.banned = 1)
// anywhere in the codebase (Ban.js, user.js, botx.js, listbanbybot.js, etc.)
// If the target is listed in config.json -> ADMINBOT, the ban is blocked and the
// "you're my boss" notice is sent. Returns true if the ban was blocked.
module.exports.guardAdminBan = function (api, targetID, threadID, messageID) {
	const adminList = (global.config.ADMINBOT || []).map(String);
	if (!adminList.includes(String(targetID))) return false;
