module.exports = function ({ api, models, Users, Threads, Currencies }) {
 ...
 const { handleReply, commands } = global.client
        const { messageID, threadID: eventThreadID, messageReply } = event;
        if (handleReply.length !== 0) {
            ...
            if (!handleNeedExec) return api.sendMessage(global.getText('handleReply', 'missingValue'), eventThreadID, messageID);
            try {
                var getText2;
                if (handleNeedExec.languages && typeof handleNeedExec.languages == 'object') 
                    ...
                    if (!reply.hasOwnProperty(global.config.language)) 
                    	return api.sendMessage(global.getText('handleCommand', 'notFoundLanguage', handleNeedExec.config.name), eventThreadID, messengeID);
                ...
                return api.sendMessage(global.getText('handleReply', 'executeError', error), eventThreadID, messageID);
            }
        }
    };
}
