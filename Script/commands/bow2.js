  ...
  onStart: async function ({ api, event, args }) {
    ...
    async function generateFrameWithFallback({ senderID, mention, credit, apiList }) {
      ...
      for (const baseApi of apiList) {
        ...
        const apiUrl = `${cleanBase}/api/frame?type=bow2&senderId=${senderID}&mentionId=${mention}&credit=${encodeURIComponent(credit)}`;
        try {
    ...
    try {
      ...
      const outPath = path.join(__dirname, `bow2_${Date.now()}.png`);
